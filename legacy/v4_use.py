...
tokenizer = AutoTokenizer.from_pretrained("t5-large", cache_dir="../sources/tokenizers")
...
# model = Transformer(
#     vocab_size=ru_tokenizer.vocab_size,
#     d_model=MODEL_DIM,
#     n_heads=N_HEADS,
...
"""def translate(text, device, ru_tokenizer):
    src_tok = ru_tokenizer(text, truncation=True, padding='max_length', max_length=MAX_LEN, return_tensors="pt")['input_ids'].squeeze()
    pad = ru_tokenizer("<|endoftext|>", return_tensors="pt")['input_ids']
    output = model(src_tok, pad, ru_tokenizer.pad_token_id, device)
    out_text = ru_tokenizer.decode(output, skip_special_tokens=True)
    return out_text"""
